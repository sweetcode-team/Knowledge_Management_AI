...
class EmbeddingsCreator:
    def __init__(self, langchainEmbeddingsModel: LangchainEmbeddingModel):
        self.langchainEmbeddingsModel = langchainEmbeddingsModel
        
    def embedDocument(self, documents: List[LangchainCoreDocuments]) -> List[List[float]]:
        return self.langchainEmbeddingsModel.embedDocument([document.page_content for document in documents])