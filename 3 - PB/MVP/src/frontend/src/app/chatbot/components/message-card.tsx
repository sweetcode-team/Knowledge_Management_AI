    ...
    return (
        <div className="pb-4">
            <div className={`flex gap-2 ${message.role === "user" && "flex-row-reverse"}`}>
                <Card className={`text-sm flex flex-col max-w-[80%] border-none px-4 py-3 ${message.role === "user" ? "bg-primary" : "bg-muted"}`}>
                    <CardContent className="p-0 break-words">
                        <p className={`word-break ${message.role === "user" ? "text-primary-foreground" : "text-secondary-foreground"}`}>
                            {message.content}
                        </p>
                    </CardContent>
                    {
            ...
            <div className={`text-xs mt-2 ${message.role === "user" ? "float-right mr-2" : "ml-2"}`}>
