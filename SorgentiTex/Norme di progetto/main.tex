...
\item \lbrack \textbf{Importanza}] classifica i requisiti in:
    \begin{itemize}
		...
		\item OP (Opzionale).
    \end{itemize}
\item \lbrack \textbf{codice}] identifica i requisiti per ogni tipologia. È composto da un unico numero progressivo univoco se il 
requisito non ha padre, mentre se si tratta di un sotto-requisito, segue il formato \textbf{[codice\_padre].[numero\_figlio]}; questa 
struttura è ricorsiva, quindi non pone un limite alla profondità della gerarchia;
\end{itemize}

\paragraph{Requisiti: Suddivisione (Funzionali, di Qualità, di Vincolo, di Prestazione)}
\begin{itemize}
...
\end{itemize}
\begin{itemize}
\item I requisiti di vincolo descrivono i limiti e le restrizioni normative/legislative che un sistema deve rispettare per soddisfare le 
...
\paragraph{Diagrammi}
I diagrammi dei casi d'uso e di attività riportati all'interno del documento (Analisi dei requisiti) devono seguire la notazione e le 
specifiche indicate da \textit{UML v2.5}.

\paragraph{Applicazione milestone SEMAT}
