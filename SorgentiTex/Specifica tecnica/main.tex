...
\begin{comment}
    requisiti per frontend

visualizza caratteristiche
        \item RF.O.10; %visualizza caratteristiche LLM
        \item RF.O.10.1;%visualizza caratteristiche LLM
        \item RF.O.10.2;%visualizza caratteristiche LLM
        \item RF.O.10.3;%visualizza caratteristiche LLM
        \item RF.O.10.4;%visualizza caratteristiche LLM
        \item RF.O.10.5; %visualizza caratteristiche LLM
        \item RF.O.11; %visualizza caratteristiche vector store
        \item RF.O.11.1;%visualizza caratteristiche vector store
        \item RF.O.11.2;%visualizza caratteristiche vector store
        \item RF.O.11.3;%visualizza caratteristiche vector store
        \item RF.O.11.4;%visualizza caratteristiche vector store
        \item RF.O.11.5%visualizza caratteristiche vector store
        \item RF.O.12;%visualizza caratteristiche embedding model
        \item RF.O.12.1;%visualizza caratteristiche embedding model
        \item RF.O.12.2;%visualizza caratteristiche embedding model
        \item RF.O.12.3;%visualizza caratteristiche embedding model
        \item RF.O.12.4;%visualizza caratteristiche embedding model
        \item RF.O.12.5;%visualizza caratteristiche embedding model
        \item RF.O.13;%visualizza caratteristiche sistema di archiviazione
        \item RF.O.13.1;%visualizza caratteristiche sistema di archiviazione
        \item RF.O.13.2;%visualizza caratteristiche sistema di archiviazione
        \item RF.O.13.3;%visualizza caratteristiche sistema di archiviazione
        \item RF.O.13.4;%visualizza caratteristiche sistema di archiviazione
        \item RF.O.13.5.%visualizza caratteristiche sistema di archiviazione
    ...
    STAGING:
    \item RF.O.16; %’utente deve poter caricare un documento nell’area di staging
    \item RF.O.16.1;% "
    \item RF.O.16.2;% "
    \item RF.O.17;% "
    \item RF.O.17.1;% "
    \item RF.O.17.2;% "
    \item RF.O.17.3;% "
    \item \item RF.O.18; %tentativo formato caricato  in area di staging non supportato
    \item RF.O.18.1;% "
     \item RF.O.19; %rimuovere un documento caricato nell’area di stagi
    \item RF.O.20; % "
     \item RF.O.21.1; % poter scegliere se sostituire con il documento caricato
    \item RF.O.21.2; % conferma sostituzione doc
    \item RF.O.21.3; % annulla sostituzione doc
    \item RF.O.21.4; % poter scegliere se sostituire con il documenti caricato
    \item RF.O.21.5; % conferma sostituzione docs
    \item RF.O.21.6; % annulla sostituzione docs
    
Visualizza lista chat e loro informazioni
 RF.O.34
    RF.O.34.1
    RF.O.34.1.1
    RF.O.34.1.2
    ...
    creare nuova chat e visualizzare errore se vuota già esiste
    RF.O.35
    RF.O.36
    RF.O.36.1

    BOH
    RF.O.27.1 Seleziona più documenti per eliminarli 
    RF.O.30.1 Seleziona più documenti per occultarli
    RF.O.32.1 Seleziona più documenti per riabilitarli
    RF.O.38.2 selezione più chat per eliminarle



     visualizzare i singoli documenti presenti nel sistema di archiviazione
  \item RF.O.15;
    \item RF.O.15.1;
    \item RF.O.15.1.1;
    \item RF.O.15.1.2;
...
RF.O.43 visualizzare il contenuto di una chat
RF.O.43.1
RF.O.43.1.1
RF.O.43.1.2

RF.D.44 rispostachatbot con riferimento a documento
RF.D.45 domande suggerite in chat vuota
RF.D.46 inviare domanda suggerita 
RF.D.46.1 seleziona dom. suggerita
RF.D.46.2 conferma invio dom. suggerita
RF.D.47 ascoltare tramite audio un messaggio
RF.D.48 copiare un messaggio


(fatta?) RF.OP.49 attiva modalità vocale
RF.OP.50 disattiva mod. vocale


RF.O.51 inserire messaggio in area di testo chat
RF.O.51.1 testuale
RF.O.51.2 vocale
RF.O.51.2.1 attiva inserimento messaggio vocale
RF.O.51.2.2 disattiva inserimento messaggio vocale
RF.O.53 reso noto mittente per ogni mess.

RF.O.54 offrire opzioni che garantiscono la privacy delle informazioni sensibili condivise dall’utente.

RF.OP.55 fallback su ai locale

RF.D.56 richiesta non pertinente->risposta cortese
...
REQUISITI DI VINCOLO SONO DA PIAZZARE? E QUELLI DI QUALITà?
...
\end{itemize}
\end{comment}
\subsubsubsection{Lista sottocomponenti}%descrizioni da chiedere con più dettaglio
\begin{itemize}
...
\begin{itemize}
    ...
    \item RF.O.26;
    \item RF.O.26.1;
    \item RF.O.26.1.1; % poter visualizzare un errore se l’eliminazione di un documento sul sistema di archiviazione fallisce.
    \item RF.O.26.2;
    \item RF.O.26.3;
    \item RF.O.27;
    \item RF.O.28;
    \item RF.O.28.1;
    \item RF.O.28.1.1; % poter visualizzare un errore se l’eliminazione di un insieme di documenti sul sistema di archiviazione fallisce.
    \item RF.O.28.2;
    \item RF.O.28.3.
...
\begin{itemize}
    ...
    \item RF.O.21;% L’utente deve poter fare l’upload nel sistema di archiviazione e nel vector database dei documenti caricati nell’area di staging
    ...
    \item RF.O.23;
    \item RF.O.23.1;
    \item RF.O.23.1.1; %L’utente deve poter visualizzare un errore se il caricameno insieme docs fallisce
    \item RF.O.23.2;
    \item RF.O.23.3.
...
\subsubsubsection{Tracciamento dei requisiti}
\begin{itemize}
    \item RF.O.24.%visualizzare contenuto di un docs
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        Implementazione del metodo astratto di AskChatbotPort per ottenere una risposta dal chatbot ad un messaggio appartenente ad una chat identificata da un ChatId. %Nel caso in cui il chatId sia vuoto e venga inoltre generata correttamente una risposta, viene creata una nuova chat tramite postgresChatORM;
...
\end{itemize}
 ...
 \subsection{ChangeConfiguration}%TODO
 \subsubsection{Diagramma delle classi}
 \subsubsection{Lista delle sottocomponenti}
...
\end{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \code{setConfiguration(userId:int, LLMModel:PostgresLLMModelType, DocumentStore:PostgresDocumentStoreType, VectorStore:PostgresVectorStoreType, EmbeddingModel:PostgresEmbeddingModelType): PostgresConfigurationOperationResponse}\\
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        %a partire da una lista di tuple, ciascuna formata da una stringa (documentId), una lista di float (embeddings del documento) e un dizionario con coppie string: Any (metadata del documento). 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        % a partire da una lista di stringhe (documentId), una lista di float (embeddings del documento) e un dizionario con coppie string: Any (metadata del documento). 
        \item \code{getRetriever(embeddingModel:LangchainEmbeddingModel): langchain.BaseRetriever}\\
...
\subsubsubsection{PostgresDocumentStoreType (Enumeration)}
...
\subsubsubsection{PostgresEmbeddingModelType (Enumeration)}
...
\subsubsubsection{PostgresLLMModelType (Enumeration)}
...
\subsubsubsection{PostgresVectorStoreType (Enumeration)}
...
\subsubsubsection{GetDocumentController}
\label{GetDocumentControllerDettaglio}
\begin{itemize}
    \item \textbf{Attributi}:
    \begin{itemize}
        \item \code{useCase: GetDocumentUseCase}.
    \end{itemize}
    \item \textbf{Metodi}:
    \begin{itemize}
        \item \code{getDocument(documentId:String): Document}\\
...
\subsubsubsection{GetDocumentFacadeService}
...
\begin{itemize}
    ...
    \item \textbf{Metodi}:
    \begin{itemize}
        \item \code{getDocument(documentId:DocumentId): Document}\\
...
\subsubsubsection{GetDocumentUseCase}
